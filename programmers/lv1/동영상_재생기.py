...
def solution(video_len, pos, op_start, op_end, commands):
    video_sec = seconds(video_len)
    pos_sec = seconds(pos)
    start_sec = seconds(op_start)
    end_sec = seconds(op_end)
    ...
    answer = string(pos_sec)

    return answer

'''
문제가 너무 불친절해서 불편했다.
min, max와 split 의 효율성을 확인햇다.
다른 사람 코드를 보니 내가 되게 잘 한 것 같다.
물론 코드를 최대한 짧게 쓰려다 보니 40분정도 걸렸다. 분발해야겠다.
'''
