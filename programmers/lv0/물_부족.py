...
# storage는 저장량 정수, usage는 지난 달 사용량 정수, change는 변화량 리스트
# total_usage가 storage보다 커지면 종료
def solution(storage, usage, change):
    total_usage = 0
    for i in range(len(change)):
...
'''
처음 total_usage가 0이라서, 처음 total_usage는 0 이 된다.

이번 달 사용량 = 전 달 사용량 + 전 달 사용량 * 변화율 이다.

이번 달 사용량 = 전 달 사용량 x (1 + 변화율) 의 공식으로 계산해야한다.
'''